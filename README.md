...
Windors下安装：
官网下载ES和Kibana：https://www.elastic.co/
...
1、启动测试，启动端口5601
2、开发工具!( Post、curl、head、谷歌浏览器插件测试! )
...
精确查询term：term查询是直接通过倒排索引指定的词条进程精确查找的!
关于分词:
1、term，直接查询精确的，比较快
...
选择高版本：https://www.elastic.co/guide/en/elasticsearch/client/java-rest/current/index.html
...
5、进入spring-boot-autoconfigure-2.4.4.jar的data里查看elasticsearch源码

找到三个可用对象，分析一下方法得出，虽然这里导入3个类，静态内部类，核心类就一个即高级客户端
...
爬虫网页（主要用jsoup，但是爬取不了视频和音频。Tika可以）
